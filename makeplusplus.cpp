...
namespace makexx {
    ...
    //////////////////////////////////////////////////////////////
        ...
        static constexpr std::string_view generated_header_default =
            R"(// This header is generated by makeplusplus.
...
#define GET_ABS_PATH(abspath)\
    ...
    void visual_studio_project::save_project_to_file(std::string_view root) {
        ...
        fprint(solution, "Microsoft Visual Studio Solution File, Format Version 12.00\n");
        std::string sln_guid = msvc_details::generate_guid();
        for (const auto target : vcxproj_guid_map_ | std::views::keys) {
            fprint(solution, "Project(\"{0:s}\") = \"{1:s}\", \"{1:s}.vcxproj\", \"{2:s}\"\nEndProject\n", sln_guid, target, vcxproj_guid_map_[target]);
        }
        fprint(solution, "Global\n    GlobalSection(SolutionConfigurationPlatforms) = preSolution\n");
        for (auto& config : solution_configs_) {
            auto[mode, plat, tag, comb] = msvc_details::extract_config(config);
            fprint(solution, "        {0:s} = {0:s}\n", tag);
        }
        fprint(solution, "    EndGlobalSection\n    GlobalSection(ProjectConfigurationPlatforms) = postSolution\n");
        for (auto& guid : vcxproj_guid_map_ | std::views::values) {
            for (auto& config : solution_configs_) {
                auto[mode, plat, tag, comb] = msvc_details::extract_config(config);
                fprint(solution, "        {0:s}.{1:s}.ActiveCfg = {1:s}\n        {0:s}.{1:s}.Build.0 = {1:s}\n", guid, tag);
            }
        }
    ...
    void make_application::generate_header_() {
        ...
        fprint(header, "{:s}", put_header_archive_to_buffer({
            {"MXX_PROJECT_ROOT", std::format("\"{:s}/\"", std::filesystem::current_path().parent_path().generic_string())}
        }));
        header.close();
        fprint(std::cout, "Header generated!\n");
    }

    void make_application::generate_project_() {
        if (argc_ < 3) {
            fprint(std::cout, "Error, must have project name argument!\n");
            return;
        }
        ...
        project.close();
        fprint(std::cout, "Project description \"{:s}\" has been written to \"{:s}\"\n", project_name, std::filesystem::current_path().generic_string());
    }
    ...
    void make_application::read_source_and_split_targets_() {
        ...
        if (argc_ < 3) {
            fprint(std::cout, "Error, must have project description path argument!\n");
            return;
        }
        ...
        if (!ifs.good()) {
            fprint(std::cout, "Error, invalid description path!\n");
            return;
        }
        ...
        project_scope >> cpod::var("mxx_project_name", mxx_project_name_);
        project_scope >> cpod::var("mxx_project_targets", mxx_project_targets_);
        project_scope >> cpod::var("mxx_project_configurations", mxx_project_configurations_);
    }
...
if (auto it = current_archive.find_variable_begin<decltype(mxx_##fn)>("mxx_"#fn); it != current_archive.content_end()) {\
            ...
            if (std::memcmp(line_cache.c_str(), "#pragma target_config ", 22) == 0) {
                ...
                if (config_count == 0) {
                    ...
                    if (std::ranges::find(mxx_project_configurations_, current_config) == mxx_project_configurations_.end()) {
                        fprint(std::cout, "You are defining a configuration namespace that has not declared in PROJECT_CONFIGURATIONS!\n");
                    }
    ...
    void make_application::generate_actual_visual_studio_project_() {
        ...
        for (auto& target : mxx_project_targets_) {
            ...
            fprint(std::cout, "VC++ Project {:s} generated!\n", target);
        }
        ...
        fprint(std::cout, "Visual Studio Solution {:s} generated!\n"
            "----------------------------------------------------------------------------------------------\n",mxx_project_name_);
    }
    ...
    int make_application::operator()() {
        ...
        if (argc_ == 1) {
            fprint(std::cout, s_hello_message);
            return 0;
        }
        // Generate project
